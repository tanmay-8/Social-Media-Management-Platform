...
mongoose.connect(MONGODB_URI)
    .then(() => {
        if (!process.env.CLOUDINARY_CLOUD_NAME || !process.env.CLOUDINARY_API_KEY || !process.env.CLOUDINARY_API_SECRET) {
            console.warn('Cloudinary not configured. Image upload endpoints will fail until CLOUDINARY_* env vars are set.');
        }
        if (!process.env.META_APP_ID || !process.env.META_APP_SECRET) {
            console.warn('⚠️  Meta API not configured. Social media posting will not work until META_APP_ID and META_APP_SECRET are set.');
        }
